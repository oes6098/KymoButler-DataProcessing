...
def process_data (direction, column_title):
    ...
    if result_arrays:
        ...
        if 'frame2frame' in column_title:
            ...
            result_df.to_csv(os.path.join(folder_path, 'kymoresults', f"{direction}_frame2framevelocity.csv"), index=False)
            print("Results saved successfully.")
        elif 'Start2end' in column_title: 
            result_df.to_csv(os.path.join(folder_path, 'kymoresults', f"{direction}_start2endvelocity.csv"), index=False)
            print("Results saved successfully.")
        else:
            result_df.to_csv(os.path.join(folder_path, 'kymoresults', direction+column_title+'.csv'), index=False)
            print("Results saved successfully.")
    else:
